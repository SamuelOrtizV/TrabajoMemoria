...
destination_path = "./datasets/silverstone67_2"
...
def flip_images(files, source_path, save_path):
    ...
    for file in sorted_files:
        ...
        # Convertir la imagen a un tensor de PyTorch y moverla a la GPU
        image_tensor = torch.from_numpy(image).to('cuda')

        # Realizar el efecto espejo horizontal en la GPU
        image_flipped_tensor = torch.flip(image_tensor, dims=[1])

        # Convertir la imagen de vuelta a un array de NumPy y moverla a la CPU
        image_flipped = image_flipped_tensor.cpu().numpy()
